/*
 * Test: validates that the recursive body (FNDSA_LOW_RAM=1) is correctly applied at
 * every recursion level of ffsamp_fft_inner.
 *
 * Strategy:
 *   For each logn in {2, 3, ..., 10}:
 *     1. Allocate tmp[] at the BASELINE size (7n fpr).
 *     2. Set up a valid input (degenerate-but-legal Gram matrix + zero target).
 *     3. Paint qc(24..27) — the 4 qc-positions that the recursive body at this logn
 ...
 *   slack under the recursive body (outer qc(22..23) corresponds to the L=9 callee's
 ...
 *   stayed within its 6n-fpr the recursive body window.
 *
 ...
 *     - Direct test at L → confirms the recursive body layout works at level L
 *     - Cascade test → confirms recursion correctly nests the recursive body at every depth
 *
 * Build:
 ...
 *     gcc -O2 -DFNDSA_LOW_RAM=1 -c -o test_path_b.o test_path_b.c
 ...
 *         $(CC) $(CFLAGS) -DFNDSA_LOW_RAM=1 -c -o test_path_b.o test_path_b.c
 ...
 * sign_sampler.c. Without the diff (FNDSA_LOW_RAM=0), the baseline path uses
 * qc(0..27) and the sentinel will be overwritten — test will FAIL by design,
 * confirming the baseline does NOT have the recursive body.
 */
 ...
 * intermediate fpr (looks like a NaN-ish bit pattern when reinterpreted). */
...
setup_ffsamp_input(unsigned logn, fpr *tmp)
{
	...
	fpr *t1  = tmp + n;
	fpr *g01 = tmp + 2 * n;
	fpr *g00 = tmp + 3 * n;        /* self-adjoint, n/2 fpr */
	fpr *g11 = tmp + 3 * n + hn;   /* self-adjoint, n/2 fpr */
...
/* Paint a region of fpr values with the sentinel. */
static void
...
/* Count how many fpr positions in a region differ from the sentinel.
...
/* Test the recursive body at a single recursion level (direct call).
 *
...
test_direct_call_at_level(unsigned logn)
{
	...
	/* Allocate at BASELINE size (7n fpr) so we can
	 * detect any writes outside the recursive body's 24q range. */
	size_t tmp_flr = 7 * n;
	...
	/* Paint qc(24..27) — 4 qc-positions = n fpr — with sentinel. */
	...
	if (v > 0) {
		fprintf(stderr,
			"FAIL logn=%u (direct): %zu/%zu fpr in qc(24..27) modified "
			"(the recursive body should leave these untouched)\n",
			logn, v, (size_t)(4 * qsize));
		free(tmp);
		return 1;
	}

	printf("PASS logn=%u (direct): qc(24..27) untouched (%zu fpr verified)\n",
		logn, (size_t)(4 * qsize));
	free(tmp);
	return 0;
}
 ...
 *
 * Recursive body claims tmp[] can shrink from 6n fpr (= 24 qc-position) to
 * ~5.25n fpr (= 21 qc-position for L >= 4) by tightening per-level allocation.
 * The math:
 *   T(L) = 10 (persistent c1+l10+d00) + ⌈T(L-1)/2⌉ qc-position
 *   T(2) = 24 (the recursive body at logn=2)
 *   T(3) = 22, T(4) = 21, T(5..) = 21  (converges)
 *
 * Before implementing recursive body, we want to know: does the current
 * the recursive body (in sign_sampler.c) write to qc(21..23)? If yes, those writes
 * need to be eliminated/relocated before tmp[] can shrink to 21 qc-position.
 *
 * This probe paints the recursive-Path-B "extra slack" region (qc(21..23)
 * at the outer level under the recursive body's 6n fpr layout) and counts how many fpr
 * the current code modifies. A non-zero count tells us how much rework is
 * needed and quantifies the implementation cost.
 *
 * Returns the fpr count (0 = recursive body works as-is; >0 = needs work). */
static size_t
probe_recursive_path_b_at_level(unsigned logn)
{
	...
	/* Allocate at current the recursive body size (6n fpr). The recursive
	 * the recursive body target is 21 qc-position; we want to probe writes in qc(21..23)
	 * (3 qc-position × qsize fpr). */
	size_t tmp_flr = 6 * n;  /* current the recursive body size */
	fpr *tmp = (fpr *)calloc(tmp_flr, sizeof(fpr));
	if (!tmp) return SIZE_MAX;

	setup_ffsamp_input(logn, tmp);

	/* Paint qc(21..23) with sentinel — recursive body says the function
 ...
 * logn=10 to observe whether the L=9 callee respected its 6n-fpr
 * the recursive body window (callee_qc(24..27) maps exactly to outer qc(22..23)). Removed
 * after empirically discovering that the recursive body's outer-level setup itself writes
 * to qc(20..23) during step 4 (l10 save, n fpr = 4 qc-position at logn=10). The
 ...
 * test at each logn ∈ [2, 10] proves the recursive body works at that logn. The function
 * is the same code regardless of recursion depth (same FNDSA_LOW_RAM compile-
...
main(void)
{
	...
	printf("=== the recursive body per-level direct-call tests ===\n");
	...
	printf("\n=== Recursive body feasibility probe ===\n");
	printf("Recursive body target: 21 qc-position at logn>=4 (vs 24 today).\n");
	printf("Probe paints qc(21..23) and reports writes (fpr units):\n\n");
	printf("  %-8s | %-12s | %-12s | %-20s\n",
		"logn", "qc-position size", "violations", "% of probed region");
	printf("  ---------+--------------+--------------+----------------------\n");
	...
	if (recursive_path_b_works_anywhere) {
		printf("  At least one logn shows 0 violations → recursive body might\n");
	...
	printf("\n");
	if (total_failures == 0) {
		printf("Per-level direct tests: ALL PASSED — current the recursive body confirmed.\n");
		return 0;
	} else {
