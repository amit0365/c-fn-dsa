...
#define F2_SELECTOR_ORDER 1
#include "mask_gadgets.h"

uint32_t
dispatch_masked_d1(const volatile uint32_t z_sh[F2_SELECTOR_NSHARES],
                   const volatile uint32_t berexp_results[4],
                   const volatile uint32_t z_candidates[4],
                   uint32_t rng_seed)
{
    return f2_sel_dispatch_masked_d1_branchless(
...
main(void)
{
    volatile uint32_t z_sh[F2_SELECTOR_NSHARES] = {2, 1};
    ...
    for (uint32_t i = 0; i < 1024; i ++) {
        z_sh[1] = i & 3u;
        z_sh[0] = 3u ^ z_sh[1];
        acc ^= dispatch_masked_d1(z_sh, berexp_results, z_candidates,
                                  0x9E3779B9u ^ i);
    }
